...
function defaultGenData(): PlantGenData {
  return {
    api_id: 0,
    image_url: null,
    images: {},
...
const model = ref<PlantCreateInput>({
  ...
  type: "",
  location: "",
  date_planted: todayYmd(),
  last_prun: todayYmd(),
  last_water: todayYmd(),
  last_fertelized: todayYmd(),
  botanical_name: "",
...
watch(genDataJson, (val) => {
  try {
    ...
    model.value.gen_data = {
      ...
      image_url: (parsed?.image_url ?? null) as string | null,
...
function mapTrefleImagesToPlantImages(
    imgs: Record<string, Array<{ image_url?: string; copyright?: string }>> | undefined,
): PlantImages {
  if (!imgs) return;
...
function mapAnyGrowthToPlantGrowth(g: Record<string, unknown> | undefined): PlantGrowth {
  if (!g) return {};
  const asNumber = (v: unknown) =>
      typeof v === "number" ? v : typeof v === "string" ? Number(v) : undefined;

  const pick = <T>(v: unknown): T | undefined => (v === null || v === undefined ? undefined : (v as T));
...
async function onPickResult(p: TreflePlant) {
  ...
  try {
    ...
    model.value.gen_data = {
      ...
      growth: mapAnyGrowthToPlantGrowth((main?.growth ?? undefined) as any),
      sources: sourcesArr,
    };
  } catch (e: any) {
    ...
    <v-row justify="center">
      <v-col cols="12" md="10" lg="8">
        <v-card rounded="lg" elevation="6">
          ...
          <v-card-subtitle>
            Felder gemäß Plant-Type (DTO) ausfüllen – gen_data wird über die Suche
            oder Defaultwerte gesetzt.
          </v-card-subtitle>
          ...
          <v-card-text>
            ...
            <!-- 2) Deine angepasste Form (ohne pref_* Felder) -->
            <v-form ref="formRef">
              <v-row>
                <v-col cols="12" md="6">
                  <v-text-field
                    ...
                    <v-expansion-panel rounded="lg">
                      ...
                      <v-expansion-panel-text>
                        <v-textarea
                            ...
                            :error-messages="genDataJsonError ? [genDataJsonError] : []"
